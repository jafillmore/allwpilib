...
bool GetVMXInterruptResourceIndexForDigitalSourceHandle(HAL_Handle digitalSourceHandle, VMXResourceIndex& int_res_index, VMXChannelInfo& channel_info, int32_t* status)
{
	...
	if (handleType == HAL_HandleEnum::DIO) {
        ...
        channel_info = port->vmx_chan_info;
        ...
        channel_info = analogInputResource->vmx_chan_info;
