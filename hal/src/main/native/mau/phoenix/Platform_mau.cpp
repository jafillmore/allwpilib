...
#define NUM_CAN_MESSAGES_IN_BATCH 10  // TODO: Tune this based on usage patterns.
	...
	int32_t CANbus_SendFrame(uint32_t messageID, const uint8_t *data, uint8_t dataSize)
	{
		...
		HAL_CAN_SendMessage(messageID | 0x80000000, data, dataSize, periodMs, &status);
	...
	{
		HAL_CANStreamMessage[NUM_CAN_MESSAGES_IN_BATCH] msgs;
    	uint32_t remainingMessagesToRead = capacity;
		uint32_t default_stream_handle = GetDefaultCANStreamHandle();
    	...
    	while (remainingMessagesToRead > 0) {
    		...
    		HAL_CAN_ReadStreamSession(default_stream_handle, msgs, currMessagesToRead, currMessagesRead, status);
    		if (status != 0) {
    	   	...
    	   	if (currMessagesRead > 0) {
    	   		for ( uint32_t i = 0; i < currMessagesRead; i++) {
    	   			toFillArray[currCANStreamMessageIndex].arbID = msgs[i].messageID & 0x1FFFFFFF;
    	            std::memcpy(toFillArray[currCANStreamMessageIndex].data, msgs[i].data, msgs[i].dataSize);
    	            toFillArray[currCANStreamMessageIndex].dlc = msgs[i].dataSize;
    	            toFillArray[currCANStreamMessageIndex].timeStampUs = msgs[i].timeStamp;
	...
	void CANComm_ReceiveMessage(uint32_t *messageID, uint32_t messageIDMask, uint8_t *data, uint8_t *dataSize, uint32_t *timeStamp, int32_t *status) {
		*messageID |= 0x80000000;
		// TODO:  can the 0x80000000 be removed?
	...
	void CANComm_OpenStreamSession(uint32_t *sessionHandle, uint32_t messageID, uint32_t messageIDMask, uint32_t maxMessages, int32_t *status) {
		messageID |= 0x80000000;
		// TODO:  can the 0x80000000 be removed?
		HAL_CAN_OpenStreamSession(sessionHandle, messageID, messageIDMask, maxMessages, status);
	}
	...
	void CANComm_ReadStreamSession(uint32_t sessionHandle, canframe_t *messages, uint32_t messagesToRead, uint32_t *messagesRead, int32_t *status) {
    	...
    	while (remainingMessagesToRead > 0) {
    		...
    		HAL_CAN_ReadStreamSession(sessionHandle, msgs, currMessagesToRead, &currMessagesRead, status);
    		if (*status != 0) {
    			return;
    		}
    	   	...
    	   	if (currMessagesRead > 0) {
    	   		for ( uint32_t i = 0; i < currMessagesRead; i++) {
    	   			...
    	   			messages[currCANStreamMessageIndex].arbID = msgs[i].messageID & 0x1FFFFFFF;
    	            std::memcpy(messages[currCANStreamMessageIndex].data, msgs[i].data, msgs[i].dataSize);
    	            messages[currCANStreamMessageIndex].dlc = msgs[i].dataSize;
    	            messages[currCANStreamMessageIndex].timeStampUs = msgs[i].timeStamp;
    	            //Don't set any flags on toFill for right now (TODO:  Check w/Omar on this part???)
    	   			currCANStreamMessageIndex++;
    	   		}
    	   	}
	...
	const char *details, const char *location, const char * callStack)
{
	...
	HAL_SendError( isError, errorCode, isLVCode, details, location, callStack, printMsg);
